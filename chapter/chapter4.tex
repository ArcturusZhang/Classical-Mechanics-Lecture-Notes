...
\end{equation}
故$\mu$的运动内秉角动量守恒，而且有$\mbf{r} \perp \mbf{L}'$，即$\mu$在垂直于内秉角动量的固定平面内运动。因此相对运动可用平面极坐标系来描述，即取$r,\theta$为广义坐标，则此时的Lagrange函数\footnote{在本章中，Lagrange函数和角动量得符号都是$L$，但角动量是矢量，其符号为$\boldsymbol{L}$，所以在不造成混淆的情形下，这两个量仍然使用相同的字母来表示。}为
\begin{equation}
	L' = \frac12 \mu \left(\dot{r}^2 + r^2 \dot{\theta}^2\right) - V(r)
	\label{chapter4:相对运动的Lagrange函数}
\end{equation}
...
根据相对运动的Lagrange函数\eqref{chapter4:相对运动的Lagrange函数}
\begin{equation*}
	L' = \frac12 \mu \left(\dot{r}^2 + r^2 \dot{\theta}^2\right) - V(r)
\end{equation*}
可得运动方程为
...
对于引力的情形，在
\begin{equation}
	r_m = -\frac{l^2}{\mu \alpha} > 0
\end{equation}
时，等效势有极小值
\begin{equation}
	U_{\mathrm{min}} = -\frac{\mu \alpha^2}{2l^2}
\end{equation}
对于斥力的情形，等效势没有极值，因此也不存在圆轨道。
...
当质点沿着轨道运动时，坐标对时间的依赖关系可以用式\eqref{t与r的关系}得到。它可以表示为下面所述的一种方便的参数形式。首先考虑椭圆轨道。对椭圆轨道，质点与力心的最大距离为
\begin{equation*}
	r_{\max} = \frac{p}{1-e}
\end{equation*}
...
\begin{theorem}[Kepler第一定律]
	行星以太阳为焦点，沿椭圆轨道运动。
\end{theorem}

实际上，通过前面几节中对轨道方程的求解，我们还知道椭圆轨道的半长轴半短轴分别为
\begin{align}
...
\begin{theorem}[Kepler第二定律]
	太阳与行星连线的扫面速度为常量，此处扫面速度定义为
	\begin{equation*}
		\frac{\mathd \boldsymbol{S}}{\mathd t} = \frac12 \mbf{r} \times \frac{\mathd\boldsymbol{r}}{\mathd t} = \frac{\mbf{L}}{2\mu} = \text{常矢量}
	\end{equation*}
\end{theorem}

Kepler第二定律即为角动量守恒的另外一种表述。

\begin{theorem}[Kepler第三定律]
	行星公转周期的平方正比于椭圆半长轴的立方，即
	\begin{equation}
		\frac{T^2}{a^3} = \text{常数}
	\end{equation}
\end{theorem}

根据第二定律中求出的面积速度，可以求出行星公转的周期
\begin{equation}
...
\end{equation}
因此Kepler第三定律是当中心天体的质量远大于行星的质量$M\gg m$时成立的一个近似定律。
...
\begin{equation*}
	\tan \mathnormal{\Theta}_1 = \frac{\sin \theta}{m_1/m_2 + \cos \theta}
\end{equation*}
...
\end{equation*}
这里只考虑了斥力情形。此时$r_{\mathrm{min}}$满足
\begin{equation*}
